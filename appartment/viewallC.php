      ...
      <div class="container">
        ...
        <div class="row block-9 justify-content-center mb-5">
          <div class="col-md-12 mb-md-5">
          	<h2 class="text-center"><br>All complaints</h2>
				<div class="container">
				  ...
				  <table class="table table-bordered table-striped">
					...
					<tbody id="myTable">
						<?php	
//---------------PHP------------------		
						$sql1="SELECT * FROM complaint WHERE ownerID<>'No Owner' AND status<>'Deleted'";
							  $result1=$connect->query($sql1);
...
//---------------PHP------------------		
					  ...
					  <tr>
						...
						<td><?php if($row['type']=="secretary" && $row1['status']!="Solved") { ?><form method="post" action="viewallC.php"><input type="hidden" name="cidu" value="<?php echo $row1['cid'] ?>"><input type="submit" name="Cresolve" value="Resolve"> </form>
...
if(isset($_POST['Cresolve']))
{
 $cid= $_POST['cidu'];
  ...
  $sql="UPDATE `complaint` SET `udate`='$udate',`status`='Solved' WHERE cid='$cid'";
 // echo $sql;
  $result=$connect->query($sql);
  if($result==TRUE)
  {
    $m="Complaint Resolved";
	echo "<script language='javascript'> alert('$m'); </script>";
	redirect("viewallC.php");
  }
