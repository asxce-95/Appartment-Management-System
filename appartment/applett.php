   			...
   			<div class="container">
				  ...
				  <table class="table table-bordered table-striped">
					...
					<tbody id="myTable">
						<?php	
//---------------PHP------------------		
						$sql1="SELECT * FROM letter WHERE status='Pending'";
							  $result1=$connect->query($sql1);
							  $num1= $result1->num_rows;
							  if($num1==0)
							  {
								?> <h5>No Letter to Approve<h5><?php 
							  }
							  else
							  {
...
//---------------PHP------------------		
							?>
					  <tr>
						<td><?php echo $row1['ownerID'] ?></a></td>
						<td><?php echo $row1['subject'] ?></td>
						<td><?php if($row['type']=="president") { ?>
							<form method="post" action="applett.php"><input type="hidden" name="mail" value="<?php echo $row1['email'] ?>"><input type="hidden" name="lid" value="<?php echo $row1['lid'] ?>"><input type="submit" name="appL" value="Approve"> </form>															
...
$sql="UPDATE `letter` SET `status` = 'Approved' WHERE `letter`.`lid` = '$lid' ";
 $result=$connect->query($sql);
  if($result==TRUE)
  {
	  $u='pdflet.php?lid='.$lid.'&mail='.$mail1;
    redirect($u);
