   			...
   			<div class="container">
				  ...
				  <table class="table table-bordered table-striped">
					...
					<tbody id="myTable">
						<?php	
//---------------PHP------------------		
						$sql1="SELECT * FROM notice WHERE status='Pending'";
							  $result1=$connect->query($sql1);
							  $num1= $result1->num_rows;
							  if($num1==0)
							  {
								?> <h5>No Notice to Approve<h5><?php 
							  }
							  else
							  {
...
//---------------PHP------------------		
							?>
					  <tr>
						<td><?php echo $row1['datetoday'] ?></a></td>
						<td><?php echo $row1['subject'] ?></td>
						<td><?php if($row['type']=="president") { ?>
							<form method="post" action="appNot.php"><input type="hidden" name="lid" value="<?php echo $row1['lid'] ?>"><input type="submit" name="appN" value="Approve"> </form>															
...
$sql="UPDATE `notice` SET `status` = 'Approved' WHERE `notice`.`lid` = '$lid' ";
